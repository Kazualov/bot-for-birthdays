...
bulshit = ["ой какая ты глупая", "ты вообще умеешь головой пользоваться", "ты уверенна что тебе вообще это надо?",
           "Это даже жалко.", "да у меня бабуля лучше решает чем ты", "Знай свое место.",
           "Просто встань уже на калени и сдайся. "]
...
async def task0(message: Message, state: FSMContext):
    if message.text.strip().lower() == "flag<scrmch>":
        await send_with_typing(message,
                               ...
                               "Я зашифровал послание шифром Виженера\nzcpgynwlvohsa\n"
                               "Введи расшифровку."
                               )
...
async def task5(message: Message, state: FSMContext):
    if message.text.strip() == "flag<314>":
        ...
        await send_with_typing(message,
                               "Ха! Даже с математикой справиться не можешь?" + bulshit[randint(0, len(bulshit) - 1)])
