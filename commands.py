...
async def cmd_start(message: Message, state=FSMContext):
    await send_with_typing(message, "…Наконец-то.\n"
                                    "Ты догадалась посмотреть в этот жалкий пакет и пришла сюда.\n"
...
async def taskE(message: Message, state: FSMContext):
    text = ("Я — кукла, лишённая сердца, но стремящаяся стать богом.\n"
            ...
            "`01010011 01000011 01010010 01001101 01000011 01001000`\n\n"
...
async def task0(message: Message, state: FSMContext):
    if message.text.strip().lower() == "flag<scrmch>":
        await send_with_typing(message,
                               ...
                               "Первое испытание:\n"
                               "Я отправил письмо. Не хочу узнать, чтобы посторонние узнали. Ключ — это я\n"
...
async def task1(message: Message, state: FSMContext):
    ...
    if message.text.strip().lower() == "flag<happybirthday>":
        await send_with_typing(message,
                               ...
                               "Теперь тащи своё тело как можно выше к небесам, где всё вокруг кажется синим.\n"
                               "Второе испытание:\n"
...
async def task4(message: Message, state: FSMContext):
    if message.text.strip().lower() == "flag<каноэ>":
        await send_with_typing(message,
                               "Ты и здесь справилась…\n\n"
                               "Последний шаг — и мы закончим это представление.\n"
                               "Там где простираются зелёные холмы, будет ждать тебя заключительное испытание.\n"
                               "Без знания математики в этом мире ничего не добиться.\n"
                               "Сиди решай, может, хоть чему-то научишься. Ответом должно быть число.\n"
                               )
...
async def task5(message: Message, state: FSMContext):
    if message.text.strip() == "flag<314>":
        ...
        await send_with_typing(message,
                               "Ха! Даже с математикой справиться не можешь? " + bullshit[randint(0, len(bullshit) -
                                                                                                  1)])
